"""Tests for src/execution/quote_freshness.py -- the Linux-side quote
freshness/future-skew validation used by `preflight` and
`SafeExecutionService` (FX Phase 0, "quote has a timestamp in the
future" investigation). Mirrors mt5_bridge/tests/test_quote_freshness.py
one for one, since the two modules are independent implementations of
the same semantics (see src/execution/quote_freshness.py's docstring for
why they aren't a shared import).

`now_utc` is always passed explicitly here so tests are deterministic
...
from src.execution.quote_freshness import QuoteFreshnessError, validate_quote_freshness
